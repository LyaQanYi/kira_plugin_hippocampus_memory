...
- `enable_recall`：关闭后只跑后台提取，不向 prompt 注入召回结果。
- `enable_profile_injection`：每轮把发言用户（群聊则为所有参与者）的画像随召回一起注入 prompt，
  让 Bot 始终知道「在跟谁说话」。默认开启，`max_profile_chars` 控制注入长度上限。
...
- `enable_search_auto_detect`：`memory_search` 未传 `entity_id` 时，用 fast LLM 猜测查询涉及哪些用户。默认关闭（多一次 LLM 调用且可能解析错）；关闭时只按显式 `entity_id` 或当前用户/群搜索。
...
- **阶段 E**（对齐 `KiraAI-lightning` 的集成行为）：每轮注入用户画像；后台提取使用**精确**
  的逐条 sender 身份（单调水位的 `SenderCache`，不再靠文本匹配猜测）；`memory_add` 走去重/合并
  流程；`memory_update`/`memory_remove` 覆盖 facts + reflections；新增只读 `memory_profile` 工具。
...
共 23 项测试，覆盖：路径管理、目录结构、TOML CRUD、SHA-256 去重、实体画像 + 昵称归档、recall 链路、衰减引擎、召回 query 信封清洗、人设视角提取，以及阶段 E 的集成修复（每轮画像注入、单调水位 sender 缓存、精确身份 `submit_exchange`、`memory_add` 去重、facts+reflections 可编辑列表）。

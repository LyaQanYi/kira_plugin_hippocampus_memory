...
class MemoryIndex:
    ...
    def __init__(self, db_path: str):
        if not db_path:
            raise ValueError("MemoryIndex requires an explicit db_path")
        ...
        if len(parts) >= 3 and parts[0] == "entities":
            ...
            for et in ("user", "group", "channel"):
                ...
                if dirname.startswith(prefix):
                    entity_type = et
                    # Dir names are percent-encoded (see paths.encode_id) so the
                    # colon in ids like "telegram:42" is filesystem-safe; decode
                    # back to the original id the SQLite rows are keyed on.
                    entity_id = unquote(dirname[len(prefix):])
                    break
    ...
    def _build_fts_query(text: str) -> str:
        ...
        # 清理 FTS5 特殊字符：白名单方式（只保留字/数字/CJK + 空白），比黑名单
        # 健壮——黑名单总会漏掉某些 FTS5 操作符（如反引号会触发 syntax error）。
        # \w 在 Python3 str 正则下含中日韩等 Unicode 词字符。

...
class TomlTreeStore:
    ...
    def __init__(self, index: MemoryIndex = None):
        ...
        self.index = index or MemoryIndex(db_path=get_index_db_path())
    ...
    def close(self) -> None:
        """Release the underlying SQLite handle. On Windows an open db file
        blocks directory removal (PermissionError), so callers that own the
        store must close it deterministically rather than wait for GC."""
        try:
            self.index.close()

...
| POST | `/decay/run` | yes | Manually run a forgetting cycle (stage C) |
| DELETE | `/memory/{mem_id}` | yes | Delete a single memory (stage C) |

## Implementation stages

This plugin is implemented in four stages — see the parent task plan.

- **Stage A (current)**: scaffolding, auto-disable, recall injection,
...
(`adapters/llm.py`, `adapters/migration.py`) is original work to bridge the
lightning-standalone code into KiraAI's plugin host.
